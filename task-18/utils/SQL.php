...
{
    $stmt = $mysqli->prepare("SELECT beid, image_name, username, created_at, category, content, title, seo_description, keywords FROM blog_entries INNER JOIN users ON uid = author; ");
    $stmt->execute();
    $result = $stmt->get_result();
...
{
    $stmt = $mysqli->prepare("SELECT beid, image_name, username, created_at, category, content, title, seo_description, keywords FROM blog_entries INNER JOIN users ON uid = author WHERE beid = ?;");
...
function create_blog_entry($mysqli, $image, $category, $title, $content, $author, $desc, $keywords)
{
    $stmt = $mysqli->prepare("INSERT INTO blog_entries(image_name, category, title, content, author, seo_description, keywords) VALUES(?, ?, ?, ?, ?, ?, ?);");

    $stmt->bind_param("ssssiss", $image, $category, $title, $content, $author, $desc, $keywords);
...
function edit_blog_entry($mysqli, $beid, $image, $category, $title, $content, $author, $desc, $keywords)
{
    $stmt = $mysqli->prepare("UPDATE blog_entries SET image_name = ?, category = ?, title = ?, content = ?, author = ?, seo_description = ?, keywords= ?  WHERE beid = ?");
    $stmt->bind_param("ssssissi", $image, $category, $title, $content, $author, $desc, $keywords, $beid);
    $stmt->execute();
